...
int main(){
	...
	for(int i = 0; i < numMsg; ++i){
		...
		//Receive from Balancer to Server 1	
		result = msgrcv(serverID, &Server, ServerOpDim, BTOS1, 0);
		if(result == -1) { perror("msgreceive balancer fallita Balancer to Server [BTOS]\n"); exit(1); }
		printf("[SERVER 1] is receiving operation [%d]  from [BALANCER]\n", result);
		printf("[SERVER 1] Receiving message successfully [%d]\n", result);
